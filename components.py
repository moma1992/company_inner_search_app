...
def display_initial_ai_message_right(): #【問題3】
    """
    AIメッセージの初期表示
    """
...
def display_initial_ai_message_left(): #【問題3】
    """
    AIメッセージの初期表示
    """
    #with st.chat_message("assistant"):
            ...
            # LLMからの回答の場合
                ...
                if message["content"]["mode"] == ct.ANSWER_MODE_1:
                    ...
                    if not "no_file_path_flg" in message["content"]:
                        ...
                        print("【2】中身を確認",message["content"]) #【問題4】

                        # 参照元のありかに応じて、適したアイコンを取得
                        icon = utils.get_source_icon(message['content']['main_file_path'])
                        # 参照元ドキュメントのページ番号が取得できた場合にのみ、ページ番号を表示
                        if "main_page_number" in message["content"]:
                            st.success(f"{message['content']['main_file_path']}（ページNo.{message['content']['main_page_number']}）", icon=icon) #【問題4】
                        else:
                        ...
                        if "sub_message" in message["content"]:
                            ...
                            for sub_choice in message["content"]["sub_choices"]:
                                ...
                                if "page_number" in sub_choice:
                                    st.info(f"{sub_choice['source']}（ページNo.{sub_choice['page_number']}）", icon=icon) #【問題4】
                                else:
...
def display_search_llm_response(llm_response):
    ...
    if llm_response["context"] and llm_response["answer"] != ct.NO_DOC_MATCH_ANSWER:
        ...
        # 「\」を「/」に変換（Windows環境でのパス表記を、Web表示用に変換）
        ...
        if "page" in llm_response["context"][0].metadata:
            #【問題4】用に中身を確認
            print("【1】中身を確認",llm_response["context"][0].metadata) #【問題4】
            # ページ番号を取得
            main_page_number = llm_response["context"][0].metadata["page"] + 1 #【問題4】
            # 「メインドキュメントのファイルパス」と「ページ番号」を表示
            st.success(f"{main_file_path}（ページNo.{main_page_number}）", icon=icon) #【問題4】
        else:
        ...
        for document in llm_response["context"][1:]:
            ...
            if "page" in document.metadata:
                # ページ番号を取得
                sub_page_number = document.metadata["page"] + 1 #【問題4】
        ...
        if "page" in llm_response["context"][0].metadata:
            content["main_page_number"] = main_page_number #【問題4】
...
def display_contact_llm_response(llm_response):
    ...
    if llm_response["answer"] != ct.INQUIRY_NO_MATCH_ANSWER:
        ...
        for document in llm_response["context"]:
            ...
            # file_infoの\を/に変換（Windows環境でのパス表記を、Web表示用に変換）
            file_path = file_path.replace("\\", "/") #【問題4】
            ...
            if "page" in document.metadata:
                # ページ番号を取得
                page_number = document.metadata["page"] + 1 #【問題4】
